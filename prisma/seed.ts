...
// Génère un nunati unique depuis le nutrav
function mkNunati(nutrav: string): string {
  return "FR82" + nutrav.padStart(6, "0");
}

async function main() {
  console.log("🌱 Début du seed - données troupeau Samuel & Céline");
  ...
  // ───────────────────────────────────────────────
  const taureauData = [
    // Saillie naturelle (présents sur exploitation)
  ...
  console.log(`✅ ${taureauData.length} taureaux créés`);
  ...
  type VachePleine = {
    nutrav: string;
    danais: string;
    dernierVelage?: string;
    pere?: string;
    dateSaillie?: string;
    dateVelagePrevue?: string;
    notes?: string;
  };
    ...
    if (v.dateSaillie && v.dateVelagePrevue) {
      ...
      await prisma.gestation.create({
        data: {
          ...
          etat: "VERT",
          dateVelagePrevue: d(v.dateVelagePrevue),
          updatedAt: new Date(),
        },
      });
    }
  }
  console.log(`✅ ${vachesPleineDonnees.length} vaches pleines créées`);

  // ───────────────────────────────────────────────
  // CREATION VACHES VIDES
  // ───────────────────────────────────────────────
  for (const v of vachesVideDonnees) {
    ...
    // Événement sanitaire métrite pour la 777
    if (v.nutrav === "777") {
  ...
  console.log(`✅ ${vachesVideDonnees.length} vaches vides créées`);
  ...
  console.log(`✅ ${veauxDonnees.length} veaux créés`);
  ...
  console.log("\n📊 Résumé import :");
  console.log(`   🐄 ${totalVaches} vaches reproductrices`);
  console.log(`   🐮 ${totalGenisses} génisses`);
  console.log(`   🐂 ${totalVeaux} veaux mâles`);
  console.log(`   🤰 ${totalGestations} gestations confirmées (VERT)`);
  console.log(`   🍼 ${totalVelages} vélages enregistrés`);
  console.log(`   🐂 ${totalTaureaux} taureaux`);
  console.log("\n✅ Seed terminé !");
}
