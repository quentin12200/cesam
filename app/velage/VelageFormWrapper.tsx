...
export default function VelageFormWrapper({ initialOpen = false, initialMere = "", initialDate, initialSexe = "", capteurs = [], numeroVeauPropose, numerosUtilises }: Props) {
  ...
  function prochainNumero(exclus: string[]) { const longueur = numeroVeauPropose.length; let numero = Number(numeroVeauPropose); const interdits = new Set([...numerosUtilises, ...exclus]); while (interdits.has(String(numero).padStart(longueur, "0"))) numero += 1; return String(numero).padStart(longueur, "0"); }
  function ajouterVeau() { setVeaux((v) => [...v, { ...vide(qualificatif === "MORT_NEE" ? "MORT_NE" : "VIVANT"), nutrav: prochainNumero(v.map((veau) => veau.nutrav)) }]); }
  ...
  function changerQualificatif(q: Qualificatif) {
    ...
    if (q === "AVORTEMENT") setVeaux([]); else if (q === "MORT_NEE") setVeaux((v) => (v.length ? v : [{ ...vide(), nutrav: numeroVeauPropose }]).map((veau) => ({ ...veau, statut: "MORT_NE" })));
    else setVeaux((v) => {
      const presents = v.length ? v : [{ ...vide(), nutrav: numeroVeauPropose }];
        ...
        {qualificatif !== "AVORTEMENT" && <div className="space-y-2.5">
          {veaux.map((veau, i) => {
            ...
            return <div key={i} className="border border-gray-200 rounded-xl p-3 space-y-2.5"><div className="flex items-center justify-between"><p className="font-semibold text-gray-800">Veau {i + 1}</p>{i > 0 && <button type="button" onClick={() => supprimerVeau(i)} className="text-xs text-gray-500 px-2 py-1.5">Supprimer</button>}</div>
              <div><input value={veau.nutrav} onChange={(e) => modifierVeau(i, { nutrav: e.target.value.toUpperCase() })} placeholder="NUTRAV (optionnel)" className={`w-full border rounded-lg px-3 py-2.5 font-mono bg-white ${veau.nutrav && !disponible ? "border-red-400" : ""}`} />{veau.nutrav && <p className={`text-xs mt-1 ${disponible ? "text-emerald-600" : "text-red-600"}`}>{disponible ? "Numéro disponible" : "Numéro déjà utilisé"}</p>}<p className="text-[11px] leading-4 text-gray-500 mt-1">Vérifiez que ce numéro appartient bien à votre série de boucles actuelle. Une nouvelle commande peut commencer avec une autre série.</p></div>
