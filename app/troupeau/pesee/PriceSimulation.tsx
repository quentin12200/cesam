...
import { formatWeightKg, type FieldSessionEntry } from "@/lib/field-weighing";
import {
  assignPriceGroup,
  generalEstimate,
  groupForEntry,
        ...
        {animals.map((entry) => {
          ...
          <label key={entry.id} className="flex min-h-14 items-center gap-3 border-x-2 border-b-2 border-black bg-white px-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <WeighingAnimalDetails entry={entry} showProgress={false} />
            </div>
            <strong>{formatWeightKg(entry.poids)}</strong>
          </label>
          );
        })}
      </div>
