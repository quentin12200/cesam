...
const sections = [
  { statut: "EN_RETARD", titre: "Retard > 3 j", couleur: "border-red-500", texte: "text-red-700", selectionnable: true },
  { statut: "EN_RETARD_LEGER", titre: "Retard 1–3 j", couleur: "border-orange-500", texte: "text-orange-700", selectionnable: true },
  { statut: "A_FAIRE", titre: "Dans la fenêtre", couleur: "border-green-500", texte: "text-green-700", selectionnable: true },
  { statut: "A_PREVOIR", titre: "Dans ≤ 7 j", couleur: "border-yellow-400", texte: "text-yellow-700", selectionnable: false },
  { statut: "TROP_TOT", titre: "Trop tôt", couleur: "border-blue-500", texte: "text-blue-700", selectionnable: false },
] as const;

const STATUTS_SELECTIONNABLES = new Set(["A_FAIRE", "EN_RETARD_LEGER", "EN_RETARD"]);
  ...
  return (
    <section className="overflow-hidden rounded-2xl bg-white shadow-sm">
      <details ref={detailsRef} className="group">
        <summary className="cursor-pointer list-none p-4">
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 flex flex-wrap gap-x-2 gap-y-1 text-xs font-semibold">
                <span className="text-blue-700">● {groupe.tropTot} trop tôt</span>
                <span className="text-yellow-700">● {groupe.bientot} sous 7 j</span>
                <span className="text-green-700">● {groupe.aFaire} dans la fenêtre</span>
                <span className="text-orange-700">● {groupe.retardLeger} retard 1–3 j</span>
                <span className="text-red-700">● {groupe.enRetard} retard &gt; 3 j</span>
              </p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-4">
            ...
            <p><span className="block text-xs text-gray-400">Besoin maintenant</span><b>{groupe.dosesNecessaires} doses</b></p>
        ...
        <div className="border-t bg-gray-50 p-3 sm:p-4">
              ...
              <div key={section.statut} className="mb-3 last:mb-0">
                ...
                <div className="divide-y rounded-xl border bg-white">
                  {lignesSection.map((ligne) => (
                    <label key={`${ligne.animalId}-${ligne.injection}`} className={`grid grid-cols-[1.5rem_1fr] gap-2 border-l-4 p-2.5 ${section.couleur} ${section.selectionnable ? "cursor-pointer" : "cursor-default"} sm:grid-cols-[1.5rem_1.2fr_1fr_1.4fr_.9fr] sm:items-center`}>
                      <input type="checkbox" disabled={!section.selectionnable} checked={selection.has(ligne.animalId)} onChange={() => basculer(ligne.animalId)} className="h-5 w-5 accent-green-700 disabled:opacity-30" />
          ...
          {lignes.length === 0 && groupe.aConfirmer.length === 0 && <p className="py-5 text-center text-sm text-gray-500">Rien à planifier pour ce vaccin.</p>}
        </div>
      </details>
      ...
      <div className="grid grid-cols-2 gap-2 border-t p-3">
        ...
        <button type="button" disabled={selectionnables.length === 0} onClick={faireSeance} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-green-700 px-3 text-center text-sm font-semibold text-white disabled:bg-gray-300"><Syringe size={17} /> {selectionnables.length === 0 ? "Rien à faire maintenant" : `Faire la séance${selection.size > 0 ? ` (${selection.size})` : ""}`}</button>
      </div>
    </section>
  );
}
