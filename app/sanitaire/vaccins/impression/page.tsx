...
function achatConseille(achats: Array<{ doses: number; nombre: number }>, perte: number, conservationConnue: boolean): string {
  ...
  if (!formats || perte <= 0) return formats;
  return `${formats} · ${perte} dose(s) ${conservationConnue ? "restante(s)" : "perdue(s)"}`;
}

function libelleStatut(statut: string): string {
  if (statut === "EN_RETARD") return "Retard > 3 j";
  if (statut === "EN_RETARD_LEGER") return "Retard 1–3 j";
  return "Dans la fenêtre";
}
...
export default async function ImpressionVaccinsPage({ searchParams }: { searchParams: Promise<{ protocole?: string }> }) {
  ...
  return (
    <main className="print-root mx-auto max-w-[1120px] bg-white p-5 text-black">
      ...
      {groupes.map((groupe) => {
        const lignes = groupe.lignes.filter((ligne) => ["A_FAIRE", "EN_RETARD_LEGER", "EN_RETARD"].includes(ligne.statut));
        return (
          <section key={groupe.protocoleId} className="vaccine-sheet">
            ...
            {lignes.length === 0 ? <p className="border p-8 text-center">Aucun animal à préparer maintenant.</p> : (
              <table className="print-table">
                <thead><tr><th className="w-8">☐</th><th>Animal / mère</th><th>Injection</th><th>Statut</th><th>Repère</th><th>Fenêtre</th><th>Groupe / localisation</th><th>Dose</th><th className="w-40">Notes</th></tr></thead>
