...
export default function ProtocoleEditor({ protocoles, medicaments }: { protocoles: Protocole[]; medicaments: Medicament[] }) {
  ...
  function choisirPremierMedicament(id: string) {
    ...
    setForm((f) => ({ ...f, nom: f.nom || code, label: f.label || med.nom, etapes: f.etapes.map((e, index) => index === 0 ? { ...e, medicaments: [{ medicamentId: med.id, voie: med.voie || "IM", preconisationId: med.preconisations.length === 1 ? med.preconisations[0].id : "", alternative: false }] } : e) }));
    setBloc("qui");
  }
    ...
    {ouvert && <div className="space-y-3 rounded-xl bg-gray-50 p-2 shadow-inner">
      ...
      <ResumeBloc titre="Étapes" resume={resumeEtapes || "Ajouter les interventions"} ouvert={bloc === "etapes"} onClick={() => setBloc("etapes")}>
        ...
        <div className="space-y-2">{form.etapes.map((e, i) => <div key={i} className="rounded-xl border p-2.5"><div className="flex gap-2"><input value={e.label} onChange={(x) => setEtape(i, { label: x.target.value })} className="min-w-0 flex-1 border-0 font-semibold outline-none" /><select value={e.cycle} onChange={(x) => setEtape(i, { cycle: x.target.value })} className="rounded border text-xs"><option value="INITIAL">Initial</option><option value="ENTRETIEN">Entretien</option></select>{form.etapes.length > 1 && <button onClick={() => setForm((f) => ({ ...f, etapes: f.etapes.filter((_, n) => n !== i) }))}><Trash2 size={15} /></button>}</div>
          ...
          <div className="mt-2 space-y-1">{e.medicaments.map((m, mi) => { const med = medicaments.find((x) => x.id === m.medicamentId); return <div key={mi} className="grid grid-cols-[1fr_4rem_auto] gap-1"><select value={m.medicamentId} onChange={(x) => { const choisi = medicaments.find((v) => v.id === x.target.value); setEtape(i, { medicaments: e.medicaments.map((v, n) => n === mi ? { ...v, medicamentId: x.target.value, voie: choisi?.voie ?? "IM", preconisationId: choisi?.preconisations.length === 1 ? choisi.preconisations[0].id : "" } : v) }); }} className="min-w-0 rounded border px-1 py-2 text-xs"><option value="">Médicament</option>{medicaments.map((x) => <option key={x.id} value={x.id}>{x.nom}</option>)}</select><select value={m.voie} onChange={(x) => setEtape(i, { medicaments: e.medicaments.map((v, n) => n === mi ? { ...v, voie: x.target.value } : v) })} className="rounded border py-2 text-xs">{VOIES_ADMINISTRATION.map((v) => <option key={v.code}>{v.code}</option>)}</select><button onClick={() => setEtape(i, { medicaments: e.medicaments.filter((_, n) => n !== mi) })}><X size={14} /></button>{med && med.preconisations.length > 0 && <select value={m.preconisationId} onChange={(x) => setEtape(i, { medicaments: e.medicaments.map((v, n) => n === mi ? { ...v, preconisationId: x.target.value } : v) })} className="col-span-2 rounded border px-1 py-2 text-xs"><option value="">Dose à choisir</option>{med.preconisations.map((p) => <option key={p.id} value={p.id}>{p.dose} {p.unite || ""}{p.categorieAnimaux ? ` · ${p.categorieAnimaux}` : ""}</option>)}</select>}</div>; })}<button type="button" onClick={() => setEtape(i, { medicaments: [...e.medicaments, { medicamentId: "", voie: "IM", preconisationId: "", alternative: false }] })} className="text-xs text-blue-700">+ Médicament</button></div>
