...
type MedicamentEtape = { medicamentId: string; voie: string; preconisationId: string; alternative: boolean; conditionnements: string };
...
export default function ProtocoleEditor({ protocoles, medicaments }: { protocoles: Protocole[]; medicaments: Medicament[] }) {
  ...
  function choisirPremierMedicament(id: string) {
    ...
    setForm((f) => ({ ...f, nom: f.nom || code, label: f.label || med.nom, etapes: f.etapes.map((e, index) => index === 0 ? { ...e, medicaments: [{ medicamentId: med.id, voie: med.voie || "IM", preconisationId: med.preconisations.length === 1 ? med.preconisations[0].id : "", alternative: false, conditionnements: med.conditionnements.map((c) => c.doses).join(", ") || PACKS_DEFAUT }] } : e) }));
    setBloc("qui");
  }
