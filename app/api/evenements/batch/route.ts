...
export async function POST(request: NextRequest) {
  try {
    ...
    const { animalIds, nutravs, date, moment, categorie, type, description, photos, constatePar } = body;
    ...
    return NextResponse.json({ count: evenements.length, _undoId: undoId, _undoDesc: desc }, { status: 201 });
  } catch (err) {
