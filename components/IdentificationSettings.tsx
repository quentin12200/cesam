...
export default function IdentificationSettings() {
  ...
  const [reference, setReference] = useState(""), [premierNunati, setPremierNunati] = useState(""), [quantite, setQuantite] = useState("");

  async function charger() { const res = await fetch("/api/identification"); if (res.ok) setLot((await res.json()).lotActif); }
  useEffect(() => { void charger(); }, []);
  async function creerLot() {
    setSaving(true); setMessage("");
    const res = await fetch("/api/identification", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reference, premierNunati, quantite: Number(quantite) }) });
    setSaving(false);
    if (res.ok) { setNouveauLot(false); setReference(""); setPremierNunati(""); setQuantite(""); setMessage("Nouveau lot activé"); await charger(); }
  ...
  return <details className="group mt-5 border-t border-gray-100 pt-4">
    ...
    <div className="space-y-3 pt-3">
      <div className="rounded-xl border border-gray-200 p-3">
        <div className="flex items-start justify-between gap-3"><div><h4 className="text-sm font-bold">Lot actif</h4>{lot ? <p className="mt-1 text-xs text-gray-500">{lot.reference || "Sans référence"}</p> : <p className="mt-1 text-xs text-gray-500">Aucun lot configuré</p>}</div><button type="button" onClick={() => setNouveauLot((v) => !v)} className="text-right text-xs font-semibold text-green-700">Ajouter un nouveau lot de boucles</button></div>
